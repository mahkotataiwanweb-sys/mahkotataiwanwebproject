    ...
    const ctx = gsap.context(() => {
      ...
      if (textRef.current) {
        ...
        if (headingBlock) {
          ...
          gsap.fromTo(
            headingChildren,
            { opacity: 0, y: 80, scale: 0.85, rotateX: -25, filter: 'blur(2px)' },
            {
        ...
        if (pillsBlock && pillsBlock.children.length >= 3) {
          ...
          Array.from(pillsBlock.children).forEach((pill, i) => {
            ...
            gsap.fromTo(
              pill,
              { opacity: 0, x: dir.x, y: dir.y, scale: 0.8, filter: 'blur(2px)' },
              {
...
<div ref={storyRef} className="max-w-5xl mx-auto px-6">
      ...
      <section className="py-20 sm:py-28 relative overflow-hidden bg-cream">
        ...
        >
          <div
            className="relative bg-navy rounded-[2rem] p-10 sm:p-14 lg:p-16 text-center overflow-hidden shadow-[0_40px_100px_-25px_rgba(0,48,72,0.35),0_0_0_1px_rgba(255,255,255,0.05)_inset]"
