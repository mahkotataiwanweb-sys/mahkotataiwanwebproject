...
import {
  Plus, Pencil, Trash2, Search, Newspaper, Images,
...
const TYPE_OPTIONS: { value: ArticleType; label: string; pill: string }[] = [
  { value: 'event', label: 'Event', pill: 'admin-pill-info' },
  { value: 'news', label: 'News', pill: 'admin-pill-accent' },
  { value: 'lifestyle', label: 'Lifestyle', pill: 'admin-pill-success' },
  { value: 'recipe', label: 'Recipe', pill: 'admin-pill-warn' },
];
...
export default function ArticlesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((a) => {
      ...
      if (q && ![a.title_en, a.title_id, a.title_zh, a.slug].some((v) => (v || '').toLowerCase().includes(q))) return false;
      return true;
    });
  ...
  const handleDelete = async (a: Article) => {
    if (!confirm(`Delete article "${a.title_en}"?`)) return;
  ...
  const columns: ColumnDef<Article>[] = [
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-14 h-10 rounded-lg overflow-hidden bg-[var(--color-admin-surface-2)] dark:bg-[var(--color-admin-surface-2-dark)] flex-shrink-0">
            {a.image_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center"><Newspaper className="w-3.5 h-3.5 text-[var(--color-admin-faint)]" /></div>
            )}
          </div>
    ...
    },
    {
      ...
      render: (a) => {
        ...
        return <span className={`admin-pill ${t?.pill || 'admin-pill-neutral'} capitalize`}>{a.type}</span>;
      },
    },
    {
      key: 'published',
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        title="Articles"
        subtitle="Blog · News · Recipe · Event · Lifestyle"
        actions={
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="admin-search-wrap">
          ...
          <AdminInput value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search articles…" />
        </div>
        <AdminSelect value={filterType} onChange={(e) => setFilterType(e.target.value as 'all' | ArticleType)}>
          <option value="all">All types ({items.length})</option>
          {TYPE_OPTIONS.map((t) => (
            <option key={t.value} value={t.value}>
              {t.label} ({items.filter((a) => a.type === t.value).length})
            </option>
          ))}
        </AdminSelect>
      </div>
      ...
      <AdminTable
        ...
        emptyState={
          <EmptyState
            title="No articles"
            description="Add your first article to populate blog/news/event sections."
            icon={<Newspaper className="w-6 h-6" />}
            action={<AdminButton variant="accent" iconLeft={<Plus className="w-4 h-4" />} onClick={openAdd}>Add Article</AdminButton>}
          />
        }
      />
      ...
      <AdminModal
        ...
        footer={
          <>
            <TranslateAllButton
              fields={[
                { base: 'title', values: form.title, context: 'article title' },
                { base: 'excerpt', values: form.excerpt, context: 'article excerpt' },
                { base: 'description', values: form.description, context: 'article description' },
                { base: 'content', values: form.content, context: 'article body content' },
              ]}
              onUpdate={(u) =>
                setForm((p) => ({
                  ...p,
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <AdminSelect value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value as ArticleType }))}>
                {TYPE_OPTIONS.map((t) => (
          ...
          <MultilingualField
            ...
            context="article short description"
          />
          <MultilingualField
            label="Body Content"
            multiline
